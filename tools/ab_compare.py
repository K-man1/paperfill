...
# Dev harness run straight from the repo (`python tools/ab_compare.py`), so put
# src/ on the path rather than requiring an editable install first.
_REPO_ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(_REPO_ROOT / "src"))

import fitz

from paperfill.ai.preprocess import preprocess_pdf, lines_in_reading_order
from paperfill.ai.multimodal_preprocess import multimodal_preprocess_pdf
from paperfill.ai.render import build_overlays_from_structure, render_overlays_pdf

# Write next to the committed samples so a rerun refreshes the evidence set
# in place, whatever directory the script was invoked from.
OUT = str(_REPO_ROOT / "docs" / "ab-comparison")
